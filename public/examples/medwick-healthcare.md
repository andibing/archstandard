...
This is a **fictional but realistic** Solution Architecture Document for Medwick Healthcare Trust's MyMedwick Patient Portal. It demonstrates the Architecture Description Standard at **Comprehensive** depth for a Tier 1 Critical clinical system governed by NHS clinical safety standards and UK data protection law. Every section is completed with realistic content showing what a mature, well-documented SAD looks like for an NHS Trust-class solution where patient harm is a credible risk.

**Fictional organisation:** Medwick Healthcare Trust -- a mid-sized NHS-style hospital trust, approximately 12,000 staff, serving 800,000 patients across three acute sites and a network of community clinics.
**Fictional solution:** MyMedwick Patient Portal -- a web and mobile application providing outpatient appointment management, test results, letters, and prescription management to patients, integrated with the Trust's Electronic Patient Record (EPR), the NHS Spine, and NHS e-Referral Service.
...
| **Classification** | Official -- Sensitive (NHS) |
...
| 0.4 | 2025-04-08 | Dr Amir Doe | Clinical safety review; Hazard Log reference added (DCB0129/0160) |
| 0.5 | 2025-04-22 | Sally Bloggs | Information Governance review; DPIA and DCB0160 status incorporated |
...
| 1.2 | 2025-11-19 | Dr Raj Doe | Updated following annual DSPT submission and NHS Digital assurance review |
...
| Priya Bloggs | Data Architect (NHS data standards, FHIR R4) | Reviewer |
...
- Integration with NHS Spine (PDS, SCR), NHS e-Referral Service, GP Connect (appointment availability only)
- Patient identity (Azure AD B2C) and clinician identity (NHS CIS2)
- Azure Communication Services SMS and email (appointment reminders, OTP, notifications)
- Clinical safety case (DCB0129) and deployment safety case (DCB0160)
...
- DCB0129 Hazard Log -- MyMedwick (MHT-HAZ-LOG-0208)
- DCB0160 Deployment Safety Case -- MyMedwick (MHT-DSC-0208)
- NHS Data Security and Protection Toolkit Submission 2025/26 (Organisation Code: RX9)
...
The solution is built on Microsoft Azure (UK South primary, UK West DR) using a layered architecture: React single-page application for the web front end; Ionic/Capacitor hybrid mobile apps; .NET 8 microservices behind Azure API Management; Azure SQL for portal-owned data; and an integration layer that exposes and consumes FHIR R4 APIs against the Trust EPR, the NHS Spine, and the NHS e-Referral Service. Patient identity is provided by Azure AD B2C with NHS-compatible identity proofing; clinician identity (for staff administration functions) is provided by NHS CIS2.

Clinical safety has been assessed under DCB0129 (manufacturer) and DCB0160 (deployment). A Clinical Safety Officer (Dr Amir Doe) has signed off the clinical safety case; the Hazard Log is actively maintained. The service is included in Medwick's annual Data Security and Protection Toolkit (DSPT) submission.

### 1.2 Business Context & Drivers

| Driver | Description | Priority |
|--------|------------|----------|
| NHS Long Term Plan -- digital-first patient access | National commitment for every patient to have digital access to their outpatient records, appointments and prescriptions | Critical |
...
|-----------|--------------------------|---------|------------------------------|
| Patient Identity | NHS login (national service) | No | Evaluated in ADR-003; rejected in favour of Azure AD B2C due to Trust control over identity proofing journey, faster enrolment, and ability to federate NHS login as an external IdP later if required |
| Clinician Identity | NHS CIS2 (Care Identity Service 2) | Yes | Mandatory for NHS staff accessing patient-identifiable data; used for admin / service desk functions |
| Spine Integration | NHS Spine (PDS, SCR) via TLS-MA | Yes | National service, mandatory for Patient Demographic Service lookups |
| e-Referral | NHS e-Referral Service (e-RS) | Yes | National service, mandatory for outpatient referral lifecycle |
| SMS / Email | Azure Communication Services (ACS) | Yes | Trust has enterprise Azure agreement; ACS supports UK data residency |
| Secure Email | NHSmail | Yes | Used for clinical correspondence notifications where appropriate |
...
- Integration with NHS Spine via HSCN/Message Exchange for Social Care and Health (MESH) and TLS-MA
- Azure AD B2C for patient identity (custom policies, MFA, step-up for sensitive actions)
- NHS CIS2 for clinician identity (service desk admin)
...
- Clinical safety (DCB0129/DCB0160) artefacts and Hazard Log
...
- NHS Spine PDS / SCR availability and interface versions
- NHS CIS2 availability for clinician authentication
- NHS e-Referral Service API availability and version
...
**What is being retained:** Cerner Millennium EPR (authoritative clinical record); NHS Spine and e-RS integrations (reused via new FHIR facade); NHSmail; NHS CIS2.
...
|----------------------|-----------|--------|
| Microsoft Azure as hosting platform | Trust enterprise Azure agreement; Azure UK South/West provide sovereign UK data residency; ISO/IEC 27018 assurance for cloud PII; NCSC/NHS England pattern alignment | All infrastructure on Azure; no AWS or GCP |
| All data (including PII and clinical data) in UK regions | UK GDPR, NHS Data Security and Protection Toolkit, and Caldicott Principle 7 (data sharing) | UK South primary, UK West DR; no data replication outside UK |
| FHIR R4 as the canonical clinical data contract | NHS England interoperability standards and forward compatibility with ICS-wide data sharing | FHIR facade microservice required; HL7v2 from EPR translated to FHIR |
| Azure AD B2C for patient identity (not NHS login) | Trust control over identity proofing, enrolment UX, and MFA policy; NHS login can be added as federated IdP in a later phase | Custom B2C policies to be maintained; explicit ADR (ADR-003) |
| DCB0129 and DCB0160 clinical safety compliance mandatory | NHS Digital's mandatory clinical safety standards for any health IT system with clinical impact | Clinical Safety Officer required; Hazard Log maintained; deployment safety case signed before go-live |
| Two-factor authentication required for all patients | UK GDPR Article 32 and NHS CIS2 assurance equivalence | SMS OTP (default), TOTP authenticator (advanced); step-up MFA for prescription requests |
...
- **Regulatory exposure:** A confidentiality breach of patient data would require ICO notification within 72 hours (UK GDPR Art 33) and trigger mandatory reporting under the NHS DSPT.
...
| Dr Amir Doe | Clinical Safety Officer | Clinical safety (DCB0129/0160), Hazard Log, clinical risk controls | Security, Data, Scenarios, Lifecycle |
...
| NHS England (external) | Digital policy and assurance | Interoperability, DSPT, DCB0129/0160 conformance, DSCRO | Data, Security, Governance |
...
| NHS Data Security and Protection Toolkit (DSPT) conformance | Sally Bloggs, Jane Bloggs, NHS England | 3.5, 6.8 |
...
| NHS Data Security and Protection Toolkit (DSPT) 2025/26 | Mandatory -- annual submission; status Standards Met | 10 DSPT assertions applicable; evidence mapped; submitted 2025-06-30 |
| DCB0129 -- Clinical Risk Management: Manufacturer | Mandatory for health IT systems with clinical impact | Clinical Safety Officer (Dr Amir Doe) appointed; Clinical Safety Case v3 approved; Hazard Log maintained |
| DCB0160 -- Clinical Risk Management: Deployment | Mandatory for deploying health IT in health / social care settings | Deployment Safety Case signed off by Trust CSO before each major release |
...
| NHS England Interoperability Standards | Mandatory for new health IT | FHIR R4 for external clinical APIs; NHS Number as primary identifier |
...
- Yes -- processes special category personal data (health) under UK GDPR Article 9. Not a regulated medical device (formally assessed). Subject to NHS-specific clinical safety standards (DCB0129 manufacturer, DCB0160 deployment).

#### Compliance Standards

| Standard | Version | Applicability |
|----------|---------|--------------|
| NHS Data Security and Protection Toolkit | 2025/26 | All sections -- security, IG, training, incident reporting |
| DCB0129 Clinical Risk Management (Manufacturer) | 2018 | Security, Scenarios, Lifecycle -- Hazard Log, clinical safety case |
| DCB0160 Clinical Risk Management (Deployment) | 2018 | Lifecycle, Governance -- deployment safety case |
| NHS England Interoperability Standards | 2025 | Integration -- FHIR R4, HL7v2 transition path |
| WCAG 2.2 | Level AA | Scenarios, Performance -- accessibility |
| NCSC Cloud Security Principles | 14 principles | Physical, Security |
| NHS England Data Protection Impact Assessment Framework | 2024 | Data, Security |
...
<Mermaid chart={`graph TD
    ...
    Clinician[Service Desk Admin - NHS CIS2] --> FD
...
| Clinician IdP | Gateway (identity) | NHS CIS2 OIDC for staff service desk admin functions | NHS CIS2 (external national service) | Security Team |
...
| NHS Spine | External | Use | PDS for patient demographic verification; SCR not directly consumed | Via HSCN |
| NHS e-Referral Service | External | Use | Read UBRN, appointment slots, worklists | |
...
<Mermaid chart={`graph LR
    ...
    FHIR -- TLS-MA over HSCN --> Spine[NHS Spine]
    FHIR -- TLS over HSCN --> eRS[e-RS]
    FHIR -- TLS + NHS CIS2 --> GPC[GP Connect]
...
| Admin API | NHS CIS2 | HTTPS / TLS 1.3 | OIDC authorization code | Clinician authentication |
...
| FHIR Facade | NHS Spine (PDS) | HTTPS / TLS 1.2-MA | Mutual TLS with Spine certificate | Spine Core via HSCN | Patient demographic lookups |
| FHIR Facade | NHS e-Referral Service | HTTPS / TLS 1.2 | OAuth 2.0 + NHS Digital assurance key | HSCN | Outpatient referral reads |
| FHIR Facade | GP Connect (Appointments) | HTTPS / TLS 1.2 | JWT + NHS CIS2 organisation certificate | HSCN | Cross-organisation appointment availability (Phase 2) |
...
| Service Desk Clinician | Admin Portal (Admin API) | HTTPS / TLS 1.3 | NHS CIS2 OIDC + MFA (smart card or FIDO2) | Front Door WAF | Read-only enrolment lookups, account suspension |
...
| Service desk admin (clinician) | Admin portal (responsive web) | NHS CIS2 (OIDC) + smart card or FIDO2 | HTTPS / TLS 1.3 |
...
| NHS Spine PDS | REST / SOAP | Consumed | XML / JSON | 3.0 | NHS Digital developer portal |
| NHS e-Referral Service | REST | Consumed | JSON (FHIR R4) | v2 | NHS Digital developer portal |
| GP Connect Appointments | REST (FHIR STU3) | Consumed | JSON (FHIR STU3) | STU3 v1.4 | NHS Digital developer portal |
...
| NHS CIS2 OIDC | REST (OIDC) | Consumed | JSON | OIDC 1.0 | NHS Digital developer portal |
...
<Mermaid chart={`graph TD
    ...
    AppSvc -. TLS-MA over HSCN .-> HSCN[NHS HSCN]
    HSCN --> Spine[NHS Spine]
    HSCN --> eRS[NHS e-Referral]`} />
...
| Attribute | Selection |
|-----------|----------|
| **Hosting Venue Type** | Public cloud (Azure) with HSCN connectivity to NHS national services |
...
| **Landing Zone Pattern** | Medwick ALZ (Azure Landing Zone) -- NHS England-aligned, hub-and-spoke with platform hub (shared connectivity, shared services) and workload spoke |
...
- [x] Azure Policy -- enforces NCSC / NHS / Medwick baselines
...
| Third-party / co-location connectivity required? | No -- all NHS national services via HSCN; no private co-location links |
| HSCN connectivity required? | Yes -- dedicated HSCN CN-SP link for NHS Spine, e-Referral Service, and GP Connect |
...
| Peak HSCN bandwidth | 50 Mb/s (Spine, e-RS, GP Connect combined) |
...
| Web Application Firewall (WAF) | Yes | Front Door Premium WAF with OWASP Core Rule Set 3.2, bot manager, custom rules (NHS Number format, block common path traversal) |
...
|-----------|-----------------|---------------|-----------------|-----------|---------------|---------------|-------------------|-----------------|---------------|
| Patient enrolment records (NHS Number, portal user ID mapping, verification evidence references) | Azure SQL | Yes | Life + 8 years (Records Management Code of Practice) | 5 GB | Restricted | Yes | Yes (health status inferred) | TDE + Always Encrypted (deterministic on NHS Number) | Customer-managed key (HSM-backed) |
...
| Audit log | ADLS Gen2 (WORM immutable) | Yes | 7 years (DSPT / NHS Records Management) | 150 GB / year | Restricted | Yes | Yes | Azure Storage SSE + immutable blob policy | Customer-managed key |
...
NHS data classification: All patient-identifiable data is "Official-Sensitive" with the NHS handling caveat "PERSONAL". The Caldicott Principles are applied to every data use.

#### 3.4.3 Data Lifecycle

| Stage | Description | Controls |
|-------|-------------|----------|
| **Creation / Ingestion** | Patient enrolment (self-service with NHS Number + PDS verification); clinical data read-through from EPR via FHIR facade; no primary clinical data created in portal | PDS demographics match; NHS Number validation (Modulus 11); consent captured and logged |
| **Processing** | Domain services resolve each request against EPR / Spine in real time, applying results release rules and consent checks | Every patient data access logged with NHS Number, user session, and purpose; Caldicott justification coded per access type |
...
| **Sharing / Transfer** | To NHS Spine (TLS-MA), to e-RS, to GP Connect (Phase 2), to ACS (patient contact details for SMS/email delivery only); no third-country transfer | Data minimisation -- only send what is required; DPIA DPIA-2025-004 assesses each flow |
...
| **Deletion / Purging** | On patient request (UK GDPR Art 17 with NHS records caveats) or at end of retention; pseudo-anonymised research aggregates may be retained | Data Retention Committee approves; tombstone records preserved for audit trail integrity |
...
Production data is never copied into non-production environments. Staging uses a masked dataset derived from EPR test data where all NHS Numbers, names, addresses, dates of birth, and contact details are replaced by realistic synthetic values using a deterministic tokenisation approach. Referential integrity (appointment-to-patient, result-to-patient) is preserved using the tokens. Approved by IG Lead (2025-04-08).
...
|------------|-----------|---------------|----------------|------------|
| NHS Spine (PDS) | NHS Number, demographic query | Restricted | TLS-MA over HSCN | Mutual TLS with Spine-issued certificate; minimum data for lookup |
| NHS e-Referral Service | Patient UBRN, appointment references | Restricted | HTTPS over HSCN | OAuth 2.0; NHS Digital assurance key |
| GP Connect | Patient NHS Number for appointment availability (Phase 2) | Restricted | HTTPS over HSCN | JWT + CIS2 organisation certificate |
...
| Question | Response |
|----------|----------|
| Does the solution support regulated activities? | Yes -- processes special category personal data (health). Subject to DCB0129/0160 clinical safety standards. Not a medical device (assessed). |
...
| **Spoofing** | Attacker impersonates Trust clinician to service desk admin portal | Stolen smart card / coerced clinician | Low | Critical | NHS CIS2 smart card + FIDO2; IP geo-restriction; session recording; behavioural analytics (Sentinel) |
...
| **Information Disclosure** | Wrong patient's record shown to another patient (misidentification) -- clinical safety hazard HAZ-04 | Logic bug in session/context handling; identity merge error | Low | Critical | Strict session-to-NHS-Number binding; automated clinical safety regression tests; manual clinical safety review per release (DCB0160) |
...
| **Elevation of Privilege** | Standard user escalates to admin | Broken access control | Low | Critical | RBAC + ABAC (patient can only see own NHS Number data, enforced at service layer and re-validated in FHIR facade); pen testing annually (NCC Group) |
...
| Access Type | Role(s) | Destination(s) | Authentication Method | Credential Protection |
|------------|---------|----------------|----------------------|----------------------|
| Service desk admin (clinician / admin) | Service Desk Admin, Read-Only Admin | Admin portal | NHS CIS2 OIDC -- smart card or FIDO2 (phishing-resistant) | CIS2-managed; Trust enforces smart card for clinical users |
...
| Control | Response |
|---------|----------|
| Does the application use SSO? | Yes -- Azure AD B2C for patients; NHS CIS2 for clinicians; Entra ID for staff/ops |
| What is the unique identifier for user accounts? | Patients: Azure AD B2C user objectId mapped to NHS Number (verified via PDS during enrolment); clinicians: CIS2 UUID |
| What is the authentication flow? | Authorization code + PKCE for all web/mobile; token exchange at BFF |
| Credential issuance | Patient: self-service enrolment with PDS demographic match + email verification + SMS verification; service desk admin: CIS2 smart card issued by NHS Registration Authority |
| Credential complexity | Patient: 12+ chars with complexity, breached-password check; clinician: CIS2 policy |
| Credential rotation | Patient: 180-day password rotation recommended (not enforced -- aligned to NCSC guidance); CIS2: NHS CIS2 policy |
...
| Password reset | Patient: self-service with registered email + SMS OTP verification of NHS Number + date of birth |
...
|------------|-------------|-------------------|---------------------|
| Patients | Fine-grained access bound to own NHS Number only | Azure SQL (enrolment table) | Self-service enrolment + PDS verification |
| Service desk admin | Admin Read-Only, Account Suspend | NHS CIS2 attributes + Trust role mapping | Via Trust RA process |
...
| Egress filtering | Azure Firewall; explicit allow-list (ACS, Microsoft Graph, Spine/HSCN targets, Apple/Google push); deny-all-else; NAT Gateway with fixed egress IPs |
...
| HSCN boundary | Dedicated HSCN CN-SP connection (2 links, diverse carriers) with NHS-approved firewalling |
...
|-----------|---------------|
| Security event logging | All authn/authz events, patient data access (with NHS Number, action, result), admin actions, privilege elevation, WAF blocks, configuration changes. Forwarded to Sentinel. |
...
| Threat intelligence | MISP feed, NHS Digital Data Security Centre (DSC) alerts ingested into Sentinel |
| Clinical safety monitoring | Dedicated Sentinel workbook with clinical-safety-relevant KPIs: SMS delivery failure rate, FHIR facade error rate against EPR, session-to-NHS-Number binding anomaly count |
...
| Attribute | Detail |
|-----------|--------|
| **Actor(s)** | Enrolled patient, FHIR Facade, Cerner EPR, NHS Spine PDS |
| **Trigger** | Any patient data read or write |
| **Pre-conditions** | Patient authenticated and enrolled |
| **Main Flow** | 1. Session JWT contains patient_nhs_number (verified at enrolment via PDS). 2. Every call to FHIR Facade asserts patient_nhs_number as both HTTP header and query parameter. 3. FHIR Facade verifies patient_nhs_number exists in PDS cache (TTL 24h) and has not been merged/retired. 4. FHIR Facade calls Cerner FHIR with NHS Number as identifier -- never with Cerner internal IDs. 5. Returned resources are verified to have matching patient identifier; mismatched responses raise MHT-HAZ-04 alert, block the request, and log to Sentinel. 6. If PDS returns "merged", Facade fetches new identifier, updates enrolment, logs clinical-safety event, and forces patient re-verification on next login. |
...
| **Status** | Accepted |
| **Date** | 2025-02-20 |
| **Context** | Downstream clinical systems (Cerner, WinPath, JAC) historically integrate via HL7v2 messaging (MLLP). NHS England's Interoperability Strategy mandates FHIR R4 for new clinical APIs. The FHIR Facade pattern allows both in parallel. The decision is whether MyMedwick's internal clinical data model should be HL7v2 (the data's native form) or FHIR R4. |
...
| **Alternatives Considered** | **HL7v2 as canonical:** Lower impedance with legacy systems but rejected: (a) Inconsistent with NHS England standards for new systems; (b) HL7v2 is poorly suited to JSON/REST consumption by modern web clients; (c) Would lock MyMedwick out of the ICS-wide FHIR-based data sharing roadmap. **Bespoke domain model:** Rejected -- reinvents the wheel; FHIR R4 is a recognised NHS standard and has mature .NET SDKs (Firely). |
...
**ADR-003: Azure AD B2C over NHS login for patient authentication**

| Field | Content |
|-------|---------|
| **Status** | Accepted (revised 2026-03-28; supersedes v1 which had proposed NHS login) |
| **Date** | 2026-03-28 |
| **Context** | Patient authentication choice. NHS login is the national patient identity service; Azure AD B2C is Microsoft's customer identity. National policy encourages NHS login adoption but does not mandate it for Trust patient portals. Evaluation criteria: control over enrolment UX, MFA options, verification journey speed, ability to step-up MFA for sensitive actions, future optionality. |
| **Decision** | Use Azure AD B2C as the primary patient IdP; design the architecture to allow NHS login to be federated in as an additional IdP in a future phase (target 2026-Q4). |
| **Alternatives Considered** | **NHS login as primary:** Recognised national identity; would avoid duplicate enrolment. Rejected for v2.0 because: (a) Trust requires explicit control over the enrolment UX to reach 800,000 patients quickly (including digitally-excluded patients who fail NHS login's identity proofing); (b) MFA method flexibility (SMS + TOTP + FIDO2) is richer in B2C; (c) Patients who fail NHS login verification would be blocked whereas Trust can provide an "in-person at the Trust" fallback verification through its service desk. A facade (AuthGateway) is in place so NHS login can be federated without breaking downstream services. **Bespoke Trust IdP:** rejected -- reinventing identity is high-risk. |
| **Consequences** | Positive: faster enrolment path for patients; Trust controls verification journey (including in-person fallback); future-proofed for NHS login federation. Negative: risk of fragmented identity between NHS login and Trust portal (mitigated by later federation); Trust bears responsibility for identity proofing to NHS-equivalent standard (documented in DPIA-2025-004). |
...
| Patient access audit | NHS Number, session, action, purpose (Caldicott), outcome | ADLS Gen2 (WORM immutable) | 7 years | Sentinel |
| Clinical safety event log | Hazard trigger events (HAZ-01..HAZ-07), clinical-safety-relevant errors | ADLS Gen2 WORM + Sentinel workbook | 25 years (per NHS clinical records) | Sentinel + Trust Clinical Safety portal |
...
| NHS Spine error rate | > 5% errors over 5 min | Teams + PagerDuty P2 | SRE on-call |
...
| Clinical safety incident handling | Triggered on any HAZ-* event; CSO paged; 15-min engagement; DCB0160 incident report within 48h; SI if actual harm (Datix) | Dr Amir Doe | Trust Clinical Safety Management System (CSMS) |
...
| Dependency details | NHS Spine: national service, published SLA; EPR: tested to 5,000 concurrent sessions; GP Connect: limited per NHS national throttles. |
...
| NHS Spine | Outage | FHIR Facade timeout | Enrolment blocked; existing user impact limited (demographics verified at login daily) | New enrolment suspended; banner shown |
...
| Data migration method | Azure Data Factory one-time extract of PKB user list (NHS Numbers only) for invitation mailing |
| Data volume | Approximately 40,000 NHS Numbers |
...
| NHS interoperability (Spine, e-RS, GP Connect) | Medium | Action -- NHS Digital partner programme training |
| Clinical safety (DCB0129/0160) | Medium | Action -- second CSO designate in training (supports succession planning) |
...
| NHS Spine / e-RS / GP Connect interface updates | NHS Digital issues are tracked via the Digital Assurance Manager; change windows coordinated |
...
| End-of-life triggers | ICS-wide patient portal adoption (possible 2028+); NHS login mandated |
...
| C-002 | DCB0129 and DCB0160 mandatory for any change with clinical impact | Regulatory | Clinical Safety Officer required; Hazard Log; Clinical Safety Case per release | 2026-03-01 |
...
| C-004 | Must comply with NHS DSPT (annual submission) | Regulatory | Extensive security and IG controls; evidence maintained continuously | 2025-06-30 (last submission) |
...
| A-004 | NHS login federation remains optional for Trust portals | Would require emergency NHS login integration | Medium | Open | Sally Bloggs | Current NHS England policy position; monitored |
...
| R-004 | Mitigate | Strict NHS Number binding; PDS verification; clinical safety control HAZ-04; automated regression test; CSO sign-off | Low | 2026-03-01 |
...
| D-002 | NHS Spine PDS availability | Inbound | Committed | NHS Digital | National service | 2026-03-01 |
| D-003 | NHS CIS2 for clinician admin auth | Inbound | Committed | NHS Digital | National service | 2026-03-01 |
| D-004 | NHS e-Referral Service API | Inbound | Resolved | NHS Digital | Integration live since 2025-09 | 2026-03-01 |
| D-005 | GP Connect Appointments | Inbound | Committed | NHS Digital | Integration live since 2025-09 (Phase 2) | 2026-03-01 |
| D-006 | HSCN CN-SP connectivity | Inbound | Resolved | Trust Network | Dual-link live | 2026-01-10 |
...
| D-009 | DCB0160 deployment safety case for each release | Outbound | In progress (continuous) | Dr Amir Doe | Approved for v2.0 | 2026-03-28 |
...
| I-002 | Two patients reported receiving SMS reminders at 03:00 due to scheduler timezone misconfiguration | Operational | Medium | Tom Doe | Scheduler corrected to Europe/London; affected patients contacted; DCB0160 incident report filed | Resolved | 2025-12-08 |
...
| ADR-003 | Azure AD B2C over NHS login for patient identity | Accepted (revised 2026-03-28) | 2026-03-28 | Identity model; NHS login federation planned |
...
| NHS DSPT 2025/26 (10 assertions) | Standards Met | Evidence package submitted 2025-06-30; scored Standards Met | 2.3, 3.5, 6.8 |
| DCB0129 (Manufacturer) | Clinical risk management (manufacturer) | CSO appointed; Clinical Safety Case v3 approved; Hazard Log MHT-HAZ-LOG-0208; regression suite | 2.3, 3.5, 3.6, 5.1 |
| DCB0160 (Deployment) | Clinical risk management (deployment) | Deployment Safety Case signed by Trust CSO per release; CSG sign-off | 5.4, 6.4 |
...
| NHS England Interop Standards | FHIR R4, NHS Number as primary identifier | FHIR Facade; NHS Number binding | 3.2, 3.5 |
...
| Caldicott Guardian | Senior person in an NHS organisation responsible for protecting the confidentiality of patient information |
| CCIO | Chief Clinical Information Officer |
| CIS2 | NHS Care Identity Service 2 -- NHS staff identity and authentication |
...
| CSO | Clinical Safety Officer -- defined in DCB0129 |
| DCB0129 | NHS Digital standard: Clinical Risk Management -- Manufacturer |
| DCB0160 | NHS Digital standard: Clinical Risk Management -- Deployment |
...
| GP Connect | NHS service providing GP information via FHIR |
| HAZ | Hazard (entry in Hazard Log) |
| HSCN | Health and Social Care Network -- private NHS network |
| ICO | Information Commissioner's Office -- UK data protection regulator |
| ICS | Integrated Care System |
| MESH | NHS Message Exchange for Social Care and Health |
...
| RA | NHS Registration Authority (manages NHS identity cards) |
| RoPA | Record of Processing Activities (UK GDPR Art 30) |
| SCR | Summary Care Record (NHS Spine) |
| SRO | Senior Responsible Officer |
| Spine | NHS Spine -- national set of health services (PDS, SCR, etc.) |
...
|----------|---------|-------------|----------|
| NHS Data Security and Protection Toolkit | 2025/26 | Mandatory annual NHS IG assessment | dsptoolkit.nhs.uk |
| DCB0129 Clinical Risk Management: Manufacturer | 2018 | NHS Digital clinical safety standard | NHS Digital |
| DCB0160 Clinical Risk Management: Deployment | 2018 | NHS Digital clinical safety standard | NHS Digital |
| NHS England Interoperability Standards | 2025 | FHIR R4 profiles and guidance | NHS England |
...
| Clinical Safety Case v3 -- MyMedwick | CSC-MHT-0208-v3 | Clinical safety case per DCB0129 | Trust CSMS |
| Hazard Log -- MyMedwick | MHT-HAZ-LOG-0208 | Live hazard register | Trust CSMS |
| Deployment Safety Case v3 -- MyMedwick | MHT-DSC-0208-v3 | Per DCB0160 | Trust CSMS |
...
| DCB0129 | Clinical Risk Management (Manufacturer) | 2018 | 3.5, 3.6, 5.1 |
| DCB0160 | Clinical Risk Management (Deployment) | 2018 | 5.4, 6 |
...
This SAD was assessed at **Comprehensive** depth. The scores below reflect a mature, well-documented architecture for a Tier 1 Critical clinical system under NHS clinical safety and information governance standards.
...
| **3.2 Integration & Data Flow** | 5 | All internal and external integrations documented with protocol/auth; ten API contracts versioned; end user access patterns; HSCN and NHS national services modelled |
| **3.3 Physical View** | 5 | Deployment diagram; compute fully specified (ASEv3); full networking including ExpressRoute, HSCN, Private Endpoints; six environments; security agents |
...
| **Overall** | **4.9** | **Comprehensive depth achieved. Exemplary documentation for a Tier 1 Critical NHS Trust clinical system.** |
