...
<div class="mermaid">graph TD; subgraph Build["Build Time"]; MDX["MDX Content (EN/FR/DE)"] --> SSG["Astro + Starlight SSG"]; Schema["JSON Schema"] --> SSG; TemplateGen["Template Generator (Node.js)"] --> SSG; SSG --> Output["Static HTML/CSS/JS + downloads"]; end; Output --> CF["Cloudflare Pages (Global CDN)"]; CF --> Users["End Users (Web Browsers)"]</div>
