...
+------------------+       HTTPS/JSON       +------------------+       TDS/TLS        +------------------+
|                  | ---------------------> |                  | ------------------> |                  |
|   React SPA      |                        |   Node.js API    |                     |   Azure SQL DB   |
|   (Frontend)     | <--------------------- |   (Express)      | <------------------ |   (employees)    |
|                  |       JSON responses   |                  |       Query results |                  |
+------------------+                        +------------------+                     +------------------+
        |                                          |
        | OIDC auth flow                           | Reads CSV from
        v                                          v
+------------------+                        +------------------+
|                  |                        |                  |
|   Entra ID       |                        |   Azure Blob     |
...
```
                   ...
                   |  | Azure SQL Database      |  |
                   |  | (Basic, 5 DTU)          |  |
                   |  | Server: sql-empdir-prod |  |
                   |  +------------------------+  |
                   |                              |
                   |  +------------------------+  |
                   |  | Azure Blob Storage      |  |
                   |  | (stempdirsync)          |  |
                   |  | Container: csv-staging  |  |
                   |  +------------------------+  |
                   |                              |
                   |  +------------------------+  |
                   |  | Azure Functions         |  |
