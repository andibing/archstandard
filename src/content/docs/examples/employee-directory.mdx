...
<Mermaid chart={`graph LR
    SPA["React SPA (Frontend)"] -->|"HTTPS/JSON"| API["Node.js API (Express)"]
    API -->|"TDS/TLS"| DB["Azure SQL DB (employees)"]
    SPA -->|"OIDC auth"| EntraID["Entra ID (SSO)"]
    API -->|"Reads CSV"| Blob["Azure Blob Storage (CSV staging)"]
...
<Mermaid chart={`graph TD
    subgraph Azure["Meridian Azure Tenant - UK South"]
        AppSvc["Azure App Service (B1 Basic) - React SPA + Node.js API"]
        SQL["Azure SQL Database (Basic, 5 DTU)"]
        Blob["Azure Blob Storage (csv-staging)"]
        Func["Azure Functions (Consumption) - CSV Import Job"]
        AppIns["Application Insights + Log Analytics"]
        AppSvc --> SQL
        Func --> Blob
        Func --> SQL
        AppSvc --> AppIns
