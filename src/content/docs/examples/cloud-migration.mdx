...
<Mermaid chart={`graph LR
    Browser["Browser (via Citrix)"] --> AppSvc["Azure App Service (PayrollPro .NET 6)"]
    AppSvc --> SQL["Azure SQL Database"]
    AppSvc --> Blob["Azure Blob Storage (payslips)"]
    AppSvc -->|"SFTP"| BACS["BACS Payment Gateway"]
    AppSvc -->|"API"| Pension["Crestfield Pension (API)"]
    AppSvc -->|"API"| HMRC["HMRC (API)"]
...
<Mermaid chart={`graph LR
    ...
    Calc -->|"Pension submission"| Pension["Crestfield API"]
    Calc -->|"RTI submission"| HMRC["HMRC API"]
...
<Mermaid chart={`graph TD
    ...
    subgraph UKWest["Azure UK West"]
        SQLReplica["Azure SQL Geo-Replica (read-only)"]
    end
