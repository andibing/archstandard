...
<div class="mermaid">graph LR; Browser["Browser (via Citrix)"] --> AppSvc["Azure App Service (PayrollPro .NET 6)"]; AppSvc --> SQL["Azure SQL Database"]; AppSvc --> Blob["Azure Blob Storage (payslips)"]; AppSvc -->|"SFTP"| BACS["BACS Payment Gateway"]; AppSvc -->|"API"| Pension["Crestfield Pension (API)"]; AppSvc -->|"API"| HMRC["HMRC (API)"]; EntraID["Entra ID (SSO + MFA)"] --> AppSvc; KeyVault["Azure Key Vault"] --> AppSvc</div>
...
<div class="mermaid">graph LR; Entry["Employee Data Entry"] --> PP["PayrollPro"]; PP --> Calc["Payroll Calculation"]; Calc -->|"Payment file"| BACS["BACS"]; Calc -->|"Pension submission"| Pension["Crestfield API"]; Calc -->|"RTI submission"| HMRC["HMRC API"]; Calc -->|"Payslip PDFs"| Blob["Blob Storage"]; Calc -->|"Finance journal"| ADF["ADF"] --> SAP["SAP"]</div>
...
<div class="mermaid">graph TD; subgraph UKSouth["Azure UK South"]; subgraph Hub["Hub VNet"]; VPN["VPN Gateway"] -->|"to on-prem"| OnPrem["On-Premises"]; end; subgraph Spoke["Spoke VNet"]; AppSvc["App Service (VNet Integration)"]; SQL["Azure SQL (Private Endpoint)"]; Blob["Blob Storage (Private Endpoint)"]; KV["Key Vault (Private Endpoint)"]; end; end; subgraph UKWest["Azure UK West"]; SQLReplica["Azure SQL Geo-Replica (read-only)"]; end; SQL -->|"geo-replication"| SQLReplica; Monitor["Azure Monitor + Log Analytics"] --> UKSouth; Monitor --> UKWest</div>
...
| A-005 | BACS, HMRC, and Crestfield integrations will function from Azure without changes to their end | Integration reconfiguration needed; potential re-certification with providers | High | Open | Sarah Chen | HMRC and Crestfield confirmed no IP allowlisting required. BACS gateway requires new SSH key registration (in progress). |
