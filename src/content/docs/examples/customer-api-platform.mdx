...
<Mermaid chart={`graph TD
    Partners["Partner Apps (OAuth 2.0 + mTLS)"] --> APIGW["API Gateway"]
    Admins["Internal Admins (Okta SSO)"] --> APIGW
    ...
    TxnSvc -->|"API"| Fraud["Fraud Detection (Featurespace ARIC)"]`} />
...
<Mermaid chart={`graph TD
    R53["Route 53"] --> WAF["AWS WAF + Shield Advanced"]
    WAF --> CF["CloudFront"]
