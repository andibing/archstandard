...
*[A C4 Container-level diagram would be inserted here showing: API Gateway fronting four microservices (Account Service, Transaction Service, Auth Service, Notification Service), backed by RDS PostgreSQL, ElastiCache Redis, and EventBridge. External integrations shown to Core Banking (Oracle DB), Fraud Detection (Featurespace ARIC), and notification channels (SES, SNS). Partner applications connect via API Gateway with OAuth 2.0 + mTLS. Internal administrators connect via Okta SSO.]*
...
*[A data flow diagram would be inserted here showing: Partner App --> API Gateway (TLS 1.3 + mTLS) --> Auth Service (OAuth 2.0 token validation) --> Account/Transaction Service --> Core Banking Adapter --> Oracle DB Read Replica (via Direct Connect). Async flows: Account/Transaction Service --> EventBridge --> SQS --> Notification Service --> SES/Partner Webhooks. All flows also emit audit events to EventBridge --> SQS --> S3.]*
...
*[A deployment diagram would be inserted here showing: AWS eu-west-2 (London) region with two Availability Zones. VPC containing: public subnets (NLB, NAT Gateways), private subnets (EKS node groups, RDS, ElastiCache), isolated subnets (Direct Connect Gateway to on-premises). API Gateway and CloudFront at the edge. AWS WAF and Shield Advanced in front. Route 53 for DNS. DR region eu-west-1 shown with pilot light configuration.]*
