...
|---------|------|-----------------|----------------------|
| 0.1 | 2024-09-15 | Sarah Chen | Initial draft with executive summary and logical view |
| 0.2 | 2024-09-28 | Sarah Chen | Added physical view, data view, security view |
| 0.3 | 2024-10-10 | James Okonkwo | Security review feedback incorporated |
| 0.5 | 2024-10-22 | Sarah Chen, Priya Sharma | Added quality attributes, governance, lifecycle |
| 1.0 | 2024-11-05 | Sarah Chen | First approved version following ARB review |
| 1.1 | 2025-01-15 | Sarah Chen | Updated cost model following reserved instance purchase |
| 1.2 | 2025-03-20 | Sarah Chen | Added fraud detection integration (Phase 2) |
| 2.0 | 2025-08-01 | Sarah Chen | Major revision: EKS upgrade to 1.29, Graviton migration, updated capacity projections |
| 2.1 | 2025-11-20 | Sarah Chen | Updated DR testing results, refreshed cost analysis |
...
| James Okonkwo | Principal Security Architect | Reviewer |
| Priya Sharma | Data Architect | Reviewer |
| Tom Bradley | SRE Lead | Reviewer |
| Dr. Helen Zhao | CTO | Approver |
| Marcus Webb | CISO | Approver |
| Fiona Gallagher | Head of Compliance | Approver |
| David Park | ARB Chair | Approver |
...
| **Project Manager** | Rachel Winters |
...
| Marcus Webb | CISO | Threat model, data protection, PCI-DSS compliance, incident response | Security View, Governance |
| Fiona Gallagher | Head of Compliance | Open Banking compliance, FCA regulations, audit trail, data sovereignty | Security View, Data View, Governance |
| Sarah Chen | Lead Solution Architect | Design integrity, standards compliance, technical debt, scalability | All views |
| James Okonkwo | Principal Security Architect | Authentication, encryption, network security, penetration testing | Security View, Physical View |
| Priya Sharma | Data Architect | Data classification, PII handling, data sovereignty, retention | Data View |
| Tom Bradley | SRE Lead | Observability, incident response, reliability, on-call | Quality Attributes, Lifecycle |
| Amir Hassan | Development Lead | Component design, API contracts, CI/CD, developer experience | Logical View, Integration & Data Flow, Lifecycle |
| Rachel Winters | Project Manager | Delivery timeline, cost, dependencies, risks | Executive Summary, Governance |
| Laura Mendez | Partner Manager | Partner onboarding experience, API availability, SLA commitments | Integration & Data Flow, Reliability |
...
| David Park | ARB Chair | Architecture standards compliance, reuse assessment, governance | All views |
...
|---------|---------------|-------------|
| Regulatory compliance (PSD2, Open Banking) | Fiona Gallagher, Dr. Helen Zhao | 1. Executive Summary, 2.3 Compliance, 3.5 Security View, 6. Governance |
| Data protection and PII handling | Marcus Webb, Priya Sharma | 3.4 Data View, 3.5 Security View |
| Platform availability and SLA | Tom Bradley, Laura Mendez, External API consumers | 4.2 Reliability, 5.5 Operations & Support |
| API performance and latency | Amir Hassan, External API consumers | 4.3 Performance, 3.2 Integration & Data Flow |
| Cost-effectiveness and budget | Dr. Helen Zhao, Finance team, Rachel Winters | 4.4 Cost Optimisation |
| Security posture and threat mitigation | Marcus Webb, James Okonkwo | 3.5 Security View |
| Partner onboarding and developer experience | Laura Mendez, External API consumers | 3.2 Integration & Data Flow, 3.6 Scenarios |
| Operational supportability | Tom Bradley | 4.1 Operational Excellence, 5.5 Operations & Support |
| Scalability for growth | Sarah Chen, Dr. Helen Zhao | 4.2 Reliability, 4.3 Performance, 3.3 Physical View |
| Migration from legacy PIL | Rachel Winters, Amir Hassan | 1.5 Current State, 5.2 Service Transition |
| Vendor lock-in | Sarah Chen, David Park | 3.1 Logical View, 5.10 Exit Planning |
...
<div class="mermaid">graph TD; Partners["Partner Apps (OAuth 2.0 + mTLS)"] --> APIGW["API Gateway"]; Admins["Internal Admins (Okta SSO)"] --> APIGW; APIGW --> AuthSvc["Auth Service"]; APIGW --> AcctSvc["Account Service"]; APIGW --> TxnSvc["Transaction Service"]; AcctSvc --> RDS["RDS PostgreSQL"]; TxnSvc --> RDS; AcctSvc --> Redis["ElastiCache Redis"]; TxnSvc --> Redis; AcctSvc --> EB["EventBridge"]; TxnSvc --> EB; EB --> NotifSvc["Notification Service"]; NotifSvc --> SES["Amazon SES"]; NotifSvc --> SNS["Amazon SNS"]; AcctSvc -->|"Direct Connect"| CoreBank["Core Banking (Oracle DB)"]; TxnSvc -->|"API"| Fraud["Fraud Detection (Featurespace ARIC)"]</div>
...
<div class="mermaid">graph LR; Partner["Partner App"] -->|"TLS 1.3 + mTLS"| APIGW["API Gateway"]; APIGW --> Auth["Auth Service (OAuth 2.0)"]; Auth --> Svc["Account/Transaction Service"]; Svc --> Adapter["Core Banking Adapter"]; Adapter -->|"Direct Connect"| Oracle["Oracle DB Read Replica"]; Svc -->|"async"| EB["EventBridge"]; EB --> SQS["SQS"]; SQS --> Notif["Notification Service"]; Notif --> SES["SES / Partner Webhooks"]; EB -->|"audit"| AuditQ["SQS (audit)"]; AuditQ --> S3["S3 (audit logs)"]</div>
...
<div class="mermaid">graph TD; R53["Route 53"] --> WAF["AWS WAF + Shield Advanced"]; WAF --> CF["CloudFront"]; CF --> APIGW["API Gateway"]; subgraph Primary["eu-west-2 (London) - 2 AZs"]; subgraph Public["Public Subnets"]; NLB["NLB"]; NAT["NAT Gateways"]; end; subgraph Private["Private Subnets"]; EKS["EKS Node Groups"]; RDS["RDS PostgreSQL (Multi-AZ)"]; ElastiCache["ElastiCache Redis"]; end; subgraph Isolated["Isolated Subnets"]; DX["Direct Connect Gateway"]; end; end; APIGW --> NLB --> EKS; EKS --> RDS; EKS --> ElastiCache; DX -->|"to on-premises"| OnPrem["Core Banking"]; subgraph DR["eu-west-1 (DR - Pilot Light)"]; RDSDR["RDS Replica"]; end; RDS -->|"cross-region replica"| RDSDR</div>
...
|-----------|-------------|-------|--------------|
| Incident response | P1: 15-min response, P2: 30-min response; follow ITIL incident management; post-incident review within 48 hours | SRE Lead (Tom Bradley) | Confluence: /ops/runbooks/incident-response |
...
| Partner communication | Status page updates within 15 minutes of confirmed incident; post-incident report to affected partners within 5 business days | Partner Manager (Laura Mendez) | Confluence: /ops/runbooks/partner-comms |
...
|----|-----------|----------------|-----------|--------|-------|----------|
| A-001 | Core banking Oracle read replicas will support 15,000 queries/s at peak | Platform cannot meet performance targets; would require caching redesign or additional read replicas | High | Closed | Priya Sharma | Load test results (TEST-2025-031) confirmed 18,000 queries/s sustained |
| A-002 | Featurespace ARIC API will maintain &lt;100ms P95 latency under our projected load | Fraud checking would increase API response time beyond P95 target; circuit breaker would bypass fraud checks more frequently | Medium | Open | Sarah Chen | Featurespace SLA contractually commits to 100ms P95 at 10,000 req/s; no independent verification at our projected 3-year volume |
| A-003 | Partner adoption will grow linearly to 80 partners over 3 years | Non-linear growth could exceed capacity plans; under-adoption would mean over-provisioned infrastructure (cost waste) | Medium | Open | Laura Mendez | Business development pipeline shows 50 partners in negotiation; growth rate tracking to plan |
...
|----|-----------|----------|----------|-----------|-------|
| R-001 | Core banking Oracle DB upgrade causes schema changes that break data replication | Technical | High | Medium | Priya Sharma |
| R-002 | Partner onboarding volume exceeds forecast, overwhelming support capacity | Operational | Medium | Medium | Laura Mendez |
| R-003 | Critical vulnerability discovered in base container image requiring emergency patching across all services | Security | High | High | James Okonkwo |
| R-004 | AWS eu-west-2 region experiences prolonged outage exceeding DR activation threshold | Operational | Critical | Low | Tom Bradley |
...
|----|-------|----------|--------|-------|----------------|--------|---------------|
| I-001 | Redis cluster failover caused 45-second cache miss spike during October maintenance window | Operational | Low | Tom Bradley | Updated maintenance procedure to pre-warm cache before failover; implemented dual-write to new primary during planned failover | Resolved | 2025-11-01 |
| I-002 | Three partners have not completed mTLS certificate renewal (certificates expiring in 60 days) | Operational | Medium | Laura Mendez | Automated renewal reminders sent at 90/60/30/7 days; partner manager directly contacting non-responsive partners; contingency: temporary API key fallback (with CISO approval) | In Progress | 2025-11-15 |
...
|------|------|------|-------------------------------|
| Lead Solution Architect | Sarah Chen | 2025-11-20 | JIRA: CAP-ARB-2025-003 (approved) |
| Principal Security Architect | James Okonkwo | 2025-11-18 | JIRA: CAP-SEC-2025-012 (approved) |
| Data Architect | Priya Sharma | 2025-11-15 | JIRA: CAP-DATA-2025-007 (approved) |
| Head of Compliance | Fiona Gallagher | 2025-11-19 | JIRA: CAP-COMP-2025-004 (approved) |
| CISO | Marcus Webb | 2025-11-19 | JIRA: CAP-SEC-2025-013 (approved) |
| CTO | Dr. Helen Zhao | 2025-11-20 | JIRA: CAP-ARB-2025-003 (approved) |
| ARB Chair | David Park | 2025-11-20 | JIRA: CAP-ARB-2025-003 (approved) |
